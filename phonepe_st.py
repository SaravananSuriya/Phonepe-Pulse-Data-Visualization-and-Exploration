...
if selected == 'Home':
    ...
    with column2:
        img = Image.open('phonepe.webp')
        st.image(img)
    st.markdown("# :violet[Guide:]")
    col1,col2 = st.columns([2,2])  
    with col1:
        img1 = Image.open('aggregated.webp')
    ...
    if choice == 'Aggregated':
        ...
        if option == 'Transaction':
            ...
            st.markdown("")
            st.markdown("") 
            cursor.execute(f"select year,quarter,state, sum(Transaction_Count)as total_tran_cnt, sum(transaction_amount)as total_tran_amt from agg_tran where Year = {year} group by state,quarter,Transaction_Type order by state")
            data =[]
            for i in cursor:
                data.append(i)
            ...
            sunburst = px.sunburst(df,path=['Year','Quarter','State','Total_Amount'],color='Total_Amount',color_continuous_scale='magenta',width = 800)#,color_continuous_midpoint=np.average(df['total_tran_amt']))#color_discrete_sequence=px.colors.qualitative.Plotly_r)#,labels='total_tran_cnt',,values='total_tran_cnt') #values='lifeExp',,color_continuous_scale='RdBu'
            ...
            st.markdown('')
            st.markdown('')
            sunburst = px.sunburst(df,path=['Year','Quarter','State','Total_Count'],color='Total_Count',color_continuous_scale='magenta',width = 800)#,color_continuous_midpoint=np.average(df['total_tran_amt']))#color_discrete_sequence=px.colors.qualitative.Plotly_r)#,labels='total_tran_cnt',,values='total_tran_cnt') #values='lifeExp',,color_continuous_scale='RdBu'
            sunburst.update_layout(margin = dict(t=0,l=0,r=0,b=0))
            # sunburst.update_traces(textinfo='label+percent entry')
            st.plotly_chart(sunburst)
            st.markdown('')
            ...
            st.markdown('')
            st.markdown('')
            sunburst = px.sunburst(df,path=['Year','Quarter','State','User_Count'],color='User_Count',color_continuous_scale='blues',width = 900,height = 500)#,color_continuous_midpoint=np.average(df['total_tran_amt']))#color_discrete_sequence=px.colors.qualitative.Plotly_r)#,labels='total_tran_cnt',,values='total_tran_cnt') #values='lifeExp',,color_continuous_scale='RdBu'
    ...
    if option == '1. Total Transaction Amount from all States':
        ...
        try:
            ...
            # fig.update_layout(mapbox_bounds={"west": -180, "east": -50, "south": 50, "north": 150})                                                                            # aggrnyl, bluered, electric, jet
            st.plotly_chart(fig)
        except:
            pass
...
if selected == 'Conclusion':       
    ...
    * but the Phonepe **User Count** is decreasing, It happen because of unavailability of data in 2023.
    * When we analyze in Quarter-wise, The **Transaction Amount** and **Transaction Count** is decreasing, because of **Quarter 3** and **Quarter 4** data were not available in 2023.
