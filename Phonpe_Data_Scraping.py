...
try:
    cursor.execute('create table phonepe.agg_user(SL_No int auto_increment primary key, State varchar(255) not null, Year YEAR not null, Quarter int not null, Brand varchar(250) not null, Count int not null, Percentage int not null)')
    conn.commit()
except mysql.connector.Error as err:
    print(f"Error: {err}")
...
try:
    cursor.execute('create table phonepe.map_tran(SL_No int auto_increment primary key, State varchar(255) not null, District varchar(250) not null, Year YEAR not null, Quarter int not null, Count int not null, Amount bigint(20) UNSIGNED not null)')
    conn.commit()
except mysql.connector.Error as err:
    print(f"Error: {err}")
